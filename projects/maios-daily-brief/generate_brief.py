...
import embeddings
import relevance
from deduplicate import SIMILARITY_THRESHOLD, deduplicate
...
def generate_markdown(
    ...
    lines = [
        ...
        f"- Duplicates found by `{metrics.get('dedupe_method', 'lexical overlap')}`",
...
def main() -> None:
    ...
    try:
        ...
        # Semantic deduplication when a local embedding model is present,
        # lexical overlap otherwise. Lexical cannot reach two write-ups of one
        # story that share little vocabulary, and on real newsletters it
        # demonstrably does not, so the brief says which one it used.
        if embeddings.available():
            print(
                f"Finding duplicates among {len(enriched)} stories "
                f"(cosine >= {duplicates.CANDIDATE_THRESHOLD}, then confirmed by model)...",
                flush=True,
            )
            score_pair, duplicate_stats = duplicates.build_pair_scorer(enriched)
            dedupe_method = f"embeddings + {relevance.OLLAMA_MODEL} confirmation"
            # The scorer already decided; it answers 1.0 or 0.0, so any
            # threshold between them carries the judgement across unchanged.
            dedupe_threshold = 0.5
        else:
            score_pair, duplicate_stats = None, {"candidates": 0, "checked": 0}
            dedupe_method = "lexical overlap"
            dedupe_threshold = SIMILARITY_THRESHOLD
        ...
        metrics = {
            ...
            "duplicate_candidates": duplicate_stats.get("candidates", 0),
            "words_saved_percent": (
        ...
        print(
            f"Duplicate detection: {dedupe_method}"
            + (
                f" ({duplicate_stats['candidates']} candidates checked)"
                if duplicate_stats.get("candidates")
                else ""
            )
        )
